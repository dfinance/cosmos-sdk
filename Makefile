...
test-unit:
	@VERSION=$(VERSION) go test -mod=readonly $(PACKAGES_NOSIMULATION) -tags='ledger test_ledger_mock' -count=1
...
swagger-build: $(modules_dir)/*
	...
	for file in $^ ; do \
		cp -r ./types ./$${file}/swagger_sdk; \
		cp -r ./x/auth ./$${file}/swagger_auth; \
		swag init --dir ./$${file} --output $(swagger_auto_dir)/$${file} --generalInfo ./module.go; \
		rm -rf ./$${file}/swagger_sdk ./$${file}/swagger_auth; \
	done

	#merge autogenerated modules swaggers
